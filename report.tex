...
\title{Shared-Memory SPSC IPC and Interrupt-Mode io\_uring:\\
...
364--371\,ns at 64\,B---a 14.8$\times$ improvement over pipe or
...
The shared-memory ring, rather than the interrupt-mode wakeup primitive,
delivers the primary gain.  The available bursty and offered-load data are
explicitly treated as “pre-frequency-pin” observations: their absolute
wakeup latencies are confounded by DVFS/C-state ramp-up and therefore do not
support a primitive-level ranking.  We provide a frequency-pinned replication
protocol and scope all completed \texttt{io\_uring} conclusions to default
interrupt mode (\texttt{flags=0}); SQPOLL is not measured.
\end{abstract}
...
\begin{enumerate}
  ...
  \item \textbf{Tail-latency characterisation.}  P50, P90, P99, and P99.9
        percentiles are reported over 100\,000 round-trips for 64\,B and
        4\,KiB configurations.  Completing the same capture at 64\,KiB and
        1\,MiB is retained as a final measurement requirement.

  \item \textbf{Explicit measurement scope.}  We distinguish measured
        interrupt-mode results from unmeasured SQPOLL behavior and identify
        CPU-frequency pinning as a gate before drawing wakeup-primitive
        conclusions from low-load traffic.
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}L{3.0cm}L{4.8cm}@{}}
    ...
    CPU Governor    & \texttt{schedutil}/DVFS in reported data; pinned replication pending \\
    \bottomrule
  \end{tabular}
\end{table}
...
\textbf{CPU tuning and frequency-pinned replication.}  The completed data
were collected with core affinity, but the bursty and offered-load runs retain
DVFS/C-state behavior.  As Gregg notes, frequency transitions can dominate
short idle-to-active intervals~\cite{gregg2020}.  Therefore, their absolute
wakeup latencies are not interpreted as a comparison of futex, eventfd, and
\texttt{io\_uring}.  Before final external submission, we will repeat every
bursty and offered-load point with the \texttt{performance} governor, Turbo
Boost disabled (\texttt{no\_turbo=1}), and the benchmark cores isolated where
available (\texttt{isolcpus}/\texttt{nohz\_full}).  The re-run will record
the active governor, no-turbo setting, and CPU affinity alongside each CSV.
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}lrrrrr@{}}
    ...
    \texttt{shm (busy\_poll)} & \textbf{23.678} & 23.903 & 25.489 & 27.373 & 23.616 $\pm$ 0.014 \\
    \texttt{shm (adaptive)}   & 23.865          & 23.969 & 25.890 & 27.400 & 23.833 $\pm$ 0.013 \\
    \texttt{shm (spin\_back)} & 24.204          & 24.298 & 25.881 & 27.637 & 24.187 $\pm$ 0.015 \\
    \texttt{shm (eventfd)}    & 27.233          & 27.457 & 29.191 & 30.642 & 27.203 $\pm$ 0.012 \\
    \texttt{shm (futex)}      & 27.256          & 27.442 & 29.167 & 30.592 & 27.245 $\pm$ 0.014 \\
    \texttt{unix\_socket}     & 28.234          & 29.166 & 31.633 & 33.583 & 28.273 $\pm$ 0.023 \\
    \texttt{shm (io\_uring)}  & 29.453          & 29.696 & 32.567 & 34.157 & 29.456 $\pm$ 0.231 \\
    \texttt{pipe}             & 35.787          & 36.279 & 39.751 & 41.719 & 36.722 $\pm$ 1.664 \\
    \texttt{posix\_mq}        & 36.668          & 37.724 & 40.410 & 42.131 & 36.776 $\pm$ 0.050 \\
    \midrule
    \multicolumn{6}{@{}l}{\emph{Message Size: 1\,MiB --- lower is better}} \\[1pt]
    \texttt{unix\_socket}     & \textbf{242.899} & 248.887 & 251.782 & 256.567 & 243.999 $\pm$ 0.132 \\
    \texttt{shm (spin\_back)} & 270.918          & 279.315 & 288.266 & 295.620 & 273.070 $\pm$ 0.188 \\
    \texttt{shm (busy\_poll)} & 270.940          & 279.953 & 290.028 & 295.720 & 273.156 $\pm$ 0.218 \\
    \texttt{shm (eventfd)}    & 277.684          & 284.325 & 292.777 & 437.131 & 279.893 $\pm$ 0.398 \\
    \texttt{shm (adaptive)}   & 278.629          & 286.129 & 293.089 & 298.172 & 280.526 $\pm$ 0.169 \\
    \texttt{shm (futex)}      & 279.286          & 287.127 & 295.742 & 306.832 & 281.271 $\pm$ 0.187 \\
    \texttt{shm (io\_uring)}  & 281.334          & 288.865 & 296.279 & 304.555 & 283.161 $\pm$ 0.176 \\
    \texttt{pipe}             & 469.093          & 474.664 & 484.434 & 568.665 & 469.923 $\pm$ 0.309 \\
...
At 64\,B, \texttt{adaptive} (0.364\,$\mu$s) and \texttt{busy\_poll}
(0.371\,$\mu$s) are 13.6$\times$ lower than any kernel-sleeping variant.  This
...
\texttt{futex} (5.103\,$\mu$s), \texttt{eventfd} (5.028\,$\mu$s), and
\texttt{io\_uring} (7.944\,$\mu$s) all converge to 5.0--7.9\,$\mu$s P50 at 64\,B.
...
The 1\,MiB row has zero rounds and is therefore reported as N/A; the actual
\texttt{fs.mqueue.msgsize\_max} setting must be recorded with any future run.
...
\textbf{(3) 1\,MiB throughput inversion and io\_uring outlier.}
Five variants sustain 8.77--9.18\,GiB/s, consistent with the fixed 64-slot
ring stalling the producer once all slots are occupied. Interrupt-mode
\texttt{io\_uring} is a separate, stable outlier at 4.66\,GiB/s; all 15 raw
runs fall between 4.50 and 4.81\,GiB/s. This result is reported explicitly
but requires targeted instrumentation before attributing it to a particular
SQE/CQE or scheduler cost.

\textbf{(4) Small-message io\_uring overhead at 64\,B.}
At 64\,B payload, \texttt{io\_uring}'s saturated throughput is 0.178\,GiB/s,
below the spinning variants (0.307--0.389\,GiB/s) and \texttt{eventfd}
(0.335\,GiB/s), although above \texttt{futex} (0.126\,GiB/s).  This variation
reflects fixed per-message coordination overhead at very small payloads. At 4\,KiB
and above, the gap is largely amortized, with all variants clustering near
18.5\,GiB/s at 4\,KiB and 27.88\,GiB/s at 64\,KiB.
...
\textbf{Interpretation and gate.}  The near-equality among kernel-sleeping
variants in the available bursty data is consistent with their shared need to
enter the kernel and be scheduled, but it is not yet a frequency-controlled
result.  The approximately millisecond-scale medians may include wakeup delay
from DVFS/C-state transitions; consequently, this paper makes no claim that
the observed parity is intrinsic.  The pinned-frequency replication described
in Section~IV is the gate for that conclusion.  Until it is complete, the
defensible result is limited to the ring-data-path advantage and the measured
interrupt-mode depth-1 overhead.
...
and POSIX MQ (10.9\,GiB/s) slightly exceed the five non-\texttt{io\_uring} SHM
variants (8.77--9.18\,GiB/s) due to fixed ring-slot recycling stalls. The
separate 4.66\,GiB/s interrupt-mode \texttt{io\_uring} result is analysed in
...
\begin{figure}[!t]
  ...
  \caption{Throughput (GiB/s) vs.\ Message Size.  SHM Ring peaks at
    ...
    Socket (11.1\,GiB/s) and POSIX MQ (10.9\,GiB/s) exceed the five
    non-\texttt{io\_uring} SHM variants (8.77--9.18\,GiB/s). Interrupt-mode
    \texttt{io\_uring} is a separate 4.66\,GiB/s result discussed in text.}
...
7.94\,$\mu$s P50, +2.8\,$\mu$s over \texttt{futex}'s 5.10\,$\mu$s, consistent
with SQE/CQE submission overhead.  The bursty observations place all three
kernel-sleeping variants in a similar range, but because they were taken
without frequency pinning, the paper does not attribute that similarity to the
primitive itself.  That attribution awaits the controlled replication.
...
P50~$\approx$~364\,ns and P99~$\leq$~0.7\,$\mu$s at 64\,B.  The 100\,\% idle
...
Use UNIX Domain Sockets.  At 1\,MiB, UNIX Socket achieves 242.9\,$\mu$s P50
vs.\ 270.9--281.3\,$\mu$s for SHM ring variants, because the kernel socket
...
POSIX MQ (10.9\,GiB/s) exceed the five non-\texttt{io\_uring} SHM variants
(8.77--9.18\,GiB/s)---is consistent with the fixed 64-slot ring geometry.
...
8.77--9.18\,GiB/s results for the non-\texttt{io\_uring} variants.
...
\textbf{Interrupt-mode io\_uring outlier.}  The 4.66\,GiB/s io\_uring
result is not explained by ring depth alone: it is approximately half the
throughput of the other SHM wakeup variants, and all 15 raw runs lie in the
narrow 4.50--4.81\,GiB/s range.  This makes a single anomalous run unlikely,
but the present measurements do not isolate whether submission/completion
processing, FIFO signaling, or scheduling is responsible.  We therefore report
the value as a stable configuration-specific observation and reserve causal
attribution for a targeted repeat with profiling and frequency pinning.
...
\begin{enumerate}
  ...
  \item \textbf{CPU frequency and idle-state confounding (submission gate).}
        The reported bursty and offered-load data were not collected with
        frequency pinning; their $\approx$1\,ms low-load wakeup values can be
        dominated by frequency ramp-up from idle C/P states.  A rerun under
        \texttt{performance}, \texttt{no\_turbo=1}, and isolated benchmark
        cores is required before treating the apparent convergence of
        kernel-sleeping variants as a primitive-level finding.

  \item \textbf{POSIX MQ coverage.}  The recorded configuration includes a
        valid 64\,KiB run.  The 1\,MiB row has zero completed rounds and is
        reported as N/A; future artifacts must record the
        \texttt{fs.mqueue.msgsize\_max} setting alongside the result.
  ...
  \item \textbf{Interrupt-mode io\_uring scope.}  All \texttt{io\_uring}
        ...
        mode (\texttt{IORING\_SETUP\_SQPOLL}) could remove the submission
        syscall but is not measured here; it requires suitable privileges and
        should be added before making a broader claim about \texttt{io\_uring}.
...
\begin{enumerate}
  \item \textbf{The shared-memory data path drives the performance gains.}
        Spinning on a cache-aligned ring delivers 364\,ns P50 at 64\,B and
        27.88\,GiB/s peak throughput---a 14.8$\times$ latency and 5.5$\times$
  ...
  \item \textbf{Interrupt-mode wakeup result is deliberately scoped.}
        ...
        modest +2.8\,$\mu$s per-event SQE submission overhead relative to
        futex.  The available bursty data show similar values for
        all kernel-sleeping variants, but the frequency-pinned replication is
        required before that observation can be promoted to a conclusion.

  \item \textbf{Spinning variants dominate sub-$\mu$s latency requirements.}
        \texttt{adaptive} is the preferred choice: it achieves 364\,ns
...
\begin{itemize}
  \item \emph{SQPOLL mode.}  Enabling \texttt{IORING\_SETUP\_SQPOLL} and
        reporting it as a separate configuration will test whether a kernel
        poll thread changes the interrupt-mode result.
