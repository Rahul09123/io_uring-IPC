...
this payload size; the experiment does not isolate a single kernel cause.
...
explanation for the approximately millisecond-scale low-load values, although
C-state exit and scheduler delay remain part of the measured end-to-end wakeup
path.  The close values for futex, eventfd, and interrupt-mode
...
not trace SQ-thread service order or per-burst scheduler alignment, we do not
attribute the disparity to an intrinsic SQPOLL wakeup advantage.
...
down to $\approx$103\,$\mu$s at 90\% load as the shorter inter-arrival gaps
reduce opportunities for deep idle and scheduler descheduling.
...
which is why the 90\%-load row ($\approx$103\,$\mu$s) falls below
...
vs.\ 267.2--281.2\,$\mu$s for SHM ring variants. The measurements establish
the configuration-level result but do not isolate a single kernel mechanism.
