...
\begin{table}[H]
  ...
  \begin{tabular}{@{}L{3.0cm}L{4.8cm}@{}}
    ...
    CPU Governor    & Pinned (\texttt{performance}, \texttt{no\_turbo=1}) \\
    \bottomrule
  \end{tabular}
\end{table}
...
\textbf{CPU tuning and frequency pinning.}  To prevent dynamic frequency scaling (DVFS)
and idle C-state/P-state ramp-up delays from confounding the wakeup primitive
comparison (as described by Gregg~\cite{gregg2020}), all benchmark runs were
executed under strictly pinned CPU frequency conditions using the \texttt{performance}
governor with Intel Turbo Boost disabled (\texttt{no\_turbo=1}) and dedicated core affinity
(\texttt{sched\_setaffinity}).  This isolates pure kernel sleeping, scheduling, and
primitive signal propagation overhead.
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}lrrrrr@{}}
    ...
    \texttt{shm (adaptive)}   & \textbf{0.364} & 0.425 & 0.485 & 0.682 & 0.358 $\pm$ 0.002 \\
    \texttt{shm (busy\_poll)} & 0.371          & 0.392 & 0.519 & 0.635 & 0.380 $\pm$ 0.002 \\
    \texttt{shm (spin\_back)} & 0.754          & 1.272 & 1.322 & 1.396 & 0.846 $\pm$ 0.003 \\
    \texttt{shm (eventfd)}    & 5.028          & 5.150 & 6.384 & 8.694 & 5.059 $\pm$ 0.004 \\
    \texttt{shm (futex)}      & 5.103          & 5.761 & 6.793 & 9.338 & 5.234 $\pm$ 0.004 \\
    \texttt{pipe}             & 5.378          & 5.475 & 7.007 & 10.990 & 5.429 $\pm$ 0.003 \\
    \texttt{unix\_socket}     & 5.474          & 7.058 & 8.075 & 11.025 & 5.814 $\pm$ 0.005 \\
    \texttt{posix\_mq}        & 5.679          & 5.784 & 7.202 & 9.653 & 5.726 $\pm$ 0.004 \\
    \texttt{shm (io\_uring)}  & 7.944          & 8.159 & 9.834 & 14.115 & 8.019 $\pm$ 0.005 \\
    \midrule
    \multicolumn{6}{@{}l}{\emph{Message Size: 4\,KiB --- lower is better}} \\[1pt]
    \texttt{shm (adaptive)}   & \textbf{2.270} & 2.338 & 2.425 & 4.937 & 2.277 $\pm$ 0.002 \\
    \texttt{shm (busy\_poll)} & 2.298          & 2.349 & 2.432 & 4.934 & 2.301 $\pm$ 0.002 \\
    \texttt{shm (spin\_back)} & 2.663          & 2.724 & 3.120 & 5.494 & 2.656 $\pm$ 0.002 \\
    \texttt{shm (futex)}      & 6.791          & 6.914 & 8.325 & 10.720 & 6.834 $\pm$ 0.003 \\
    \texttt{shm (eventfd)}    & 6.816          & 7.124 & 9.064 & 11.412 & 7.090 $\pm$ 0.004 \\
    \texttt{pipe}             & 7.221          & 7.511 & 8.999 & 13.128 & 7.323 $\pm$ 0.003 \\
    \texttt{posix\_mq}        & 8.746          & 9.028 & 10.493 & 13.275 & 8.813 $\pm$ 0.004 \\
    \texttt{shm (io\_uring)}  & 9.783          & 9.994 & 11.907 & 15.702 & 9.838 $\pm$ 0.005 \\
    \texttt{unix\_socket}     & 11.940         & 12.618 & 13.370 & 19.123 & 11.463 $\pm$ 0.035 \\
    \midrule
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}L{2.6cm}cccc@{}}
    ...
    \midrule
    \texttt{busy\_poll}    & 0.339 & 18.77 & \textbf{27.88} & 9.17 \\
    \texttt{spin\_backoff} & 0.389 & 18.75 & 27.52 & 9.18 \\
    \texttt{adaptive}      & 0.307 & \textbf{18.89} & 27.10 & 9.06 \\
    \texttt{eventfd}       & 0.335 & 18.46 & 26.83 & 8.77 \\
    \texttt{futex}         & 0.126 & 17.97 & 26.69 & 8.79 \\
    \texttt{io\_uring}     & 0.178 & 18.45 & 27.75 & 4.66 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\textbf{The central ablation finding under frequency pinning:}  With CPU frequency
pinned to eliminate dynamic C-state/P-state frequency scaling, \texttt{io\_uring}'s
bursty wakeup latency (1058.5\,$\mu$s P50) remains statistically indistinguishable
from \texttt{futex} (1088.8\,$\mu$s) and \texttt{eventfd} (1097.2\,$\mu$s).  All three
follow equivalent kernel paths: store a flag, issue a syscall, suspend in the
scheduler, receive a wakeup interrupt, and return to userspace.  Frequency pinning
confirms that the wakeup latency parity across kernel-sleeping primitives is an
intrinsic architectural finding rather than a DVFS measurement artifact.
...
\begin{enumerate}
  ...
  \item \textbf{CPU frequency pinning protocol.}  All reported ablation and
        ping-pong measurements were gathered under CPU frequency-pinned
        conditions (\texttt{performance} governor, \texttt{no\_turbo=1}).
        Unpinned dynamic scaling (\texttt{schedutil}) introduces additional
        $\approx$1\,ms frequency ramp-up latency across all kernel-sleeping variants.
