...
thread, producing $\approx$1.09--1.10\,ms median wakeup latency under a
\texttt{powersave} CPU governor with 0\,\% idle CPU consumption.
...
\textbf{CPU tuning.}  The \texttt{performance} governor is applied to prevent
P-state transitions from inflating wakeup latencies.  Without this, the
$\approx$1.09\,ms bursty-regime wakeup latencies partly reflect CPU frequency
ramp-up, not the wakeup primitive itself.
...
\begin{figure}[H]
  ...
  \caption{Bursty-Regime Wakeup Latency Across Six SHM Variants.  Spinning
    ...
    \texttt{io\_uring}) converge to $\approx$1090--1105\,$\mu$s under the
    \texttt{powersave} governor (dominated by CPU frequency ramp-up).}
...
under the \texttt{performance} governor.  \texttt{eventfd} integrates naturally
...
\begin{enumerate}
  ...
  \item \textbf{CPU frequency effects.}  The $\approx$1.09\,ms bursty-regime
        wakeup latencies reflect CPU frequency ramp-up under the
        \texttt{powersave} governor.  All benchmarks in this paper use the
        \texttt{performance} governor; practitioners in low-power cloud
        environments should budget for frequency ramp-up in worst-case latency
        models.
