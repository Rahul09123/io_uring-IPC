...
Table~\ref{tab:pingpong} reports single-trip latency distributions (P50, P90, P99, P99.9, and mean $\pm$ 95\% CI) at 64\,B (100\,000 rounds), 4\,KiB (50\,000 rounds), 64\,KiB (10\,000 rounds), and 1\,MiB (2\,000 rounds). Values are drawn consistently from the root \texttt{data/pingpong\_*\_summary.csv} files; legacy duplicates and the conflicting aggregate file are not used.
...
This value, rather than the repository's older 28.17\,GiB/s direct-harness
result, is used for the paper's throughput conclusions.  Section~\ref{sec:provenance}
explains why the two harnesses cannot be combined.
...
\subsection{Controlled Throughput Provenance}
\label{sec:provenance}

The repository retains two throughput-data families.  The legacy direct
transport harness uses a fixed 2\,GiB transfer per size and separate transport
source trees; its \texttt{io\_uring\_results.csv} records 28.17\,GiB/s at
64\,KiB.  The controlled ablation uses the bit-identical ring, size-scaled
16/128/512\,MiB runs, and additional wakeup telemetry; it records
13.37\,GiB/s for interrupt-mode \texttt{io\_uring} at the same size.  The
legacy CSV predates the fixed-frequency environment records, so the observed
2.1$\times$ difference cannot be assigned to a wakeup mechanism or to one
harness feature.  Consequently, the final throughput conclusions and
Figure~\ref{fig:throughput} use only the refreshed controlled ablation.  The
legacy direct CSVs remain in the artifact for provenance but are not used for
cross-harness performance claims.  Kernel-mediated transports are compared
against the ring using the controlled depth-1 latency suite in
Table~\ref{tab:pingpong}.
...
Aeron's media driver supports dedicated sender, receiver, and conductor
threads with configurable idle strategies~\cite{aeron-media-driver}. A
busy-spin configuration makes a trade-off operationally similar to
\texttt{busy\_poll}: our data show 0.022\,$\mu$s wakeup latency versus
1092--1095\,$\mu$s for the interrupt-mode sleeping variants, at the cost of a
dedicated CPU resource. This comparison concerns the busy-spin trade-off; it
does not claim that every Aeron deployment uses the same strategy.
...
\begin{enumerate}
  ...
  \item \textbf{Tail-percentile provenance.}  Table~\ref{tab:pingpong} is
        ...
        are excluded.  POSIX MQ has no valid 1\,MiB result.  The 1\,MiB
...
\bibitem{aeron-media-driver}
Aeron Project,
``Media Driver,'' Aeron Documentation, accessed Aug.\ 2026.
Available: \url{https://aeron.io/docs/aeron/media-driver/}
