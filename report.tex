...
\textbf{Supplementary SQPOLL replication.}  The completed Suite~2 SQPOLL
configuration was run separately on a native Linux host (Linux 6.17.0-23),
with logical CPUs~1 and~2 pinned,
...
configuration completes under the documented protocol, but its absolute
latency is not used to rank mechanisms measured on the i5-12500H reference
host.
...
\begin{table}[!t]
  \centering
\caption{Single-Trip Ping-Pong Latency Distributions ($\mu$s). Sample counts are 100\,000 (64\,B), 50\,000 (4\,KiB), 10\,000 (64\,KiB), and 2\,000 (1\,MiB). Each row reports P50, P90, P99, P99.9, and mean $\pm$ 95\% CI from the canonical root summary CSVs. Rows marked $^\dagger$ are the supplementary frequency-controlled SQPOLL run on a separate Linux host and are not used for cross-host rankings.}
  \label{tab:pingpong}
  \resizebox{\columnwidth}{!}{%
  \small
  \begin{tabular}{@{}lrrrrr@{}}
    ...
    \texttt{shm (io\_uring SQPOLL)}$^\dagger$ & 28.079 & 28.790 & 1283.530 & 3309.630 & 55.317 $\pm$ 1.621 \\
    \midrule
    ...
    \texttt{shm (io\_uring SQPOLL)}$^\dagger$ & 31.298 & 32.053 & 1700.290 & 3353.340 & 61.712 $\pm$ 2.406 \\
    \midrule
    ...
    \texttt{shm (io\_uring SQPOLL)}$^\dagger$ & 59.188 & 61.154 & 2713.710 & 3497.240 & 116.195 $\pm$ 7.386 \\
    \midrule
    ...
    \texttt{shm (io\_uring SQPOLL)}$^\dagger$ & 620.232 & 2840.710 & 3721.520 & 4087.680 & 1132.960 $\pm$ 42.425 \\
    \bottomrule
  \end{tabular}}
\end{table}
...
\textbf{(5) SQPOLL protocol coverage is complete, but cross-host comparison is excluded.}
The supplementary SQPOLL run completed all size-scaled Suite~2 samples,
including 10\,000 rounds at 64\,KiB and 2\,000 at 1\,MiB.  Its substantially
higher P50 values and millisecond-scale tails are reported for provenance, but
the host differs from the i5-12500H reference host; therefore these
values do not support a claim that SQPOLL is faster or slower than
interrupt-mode \texttt{io\_uring}, \texttt{futex}, or \texttt{eventfd} in
...
\begin{enumerate}
  ...
  \item \textbf{SQPOLL scope.}  SQPOLL is evaluated as a separate 64-slot,
        ...
        completed Suite~2 SQPOLL replication is from a separately documented
        Linux host and is not used for cross-host rankings.  Its
