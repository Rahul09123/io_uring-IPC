...
\title{Shared-Memory SPSC IPC: A Controlled Study of Wakeup Mechanisms and SQPOLL-Assisted Signaling}
...
2.71--2.86\,$\mu$s. Modern high-performance systems reduce this cost by employing
...
0.211 and 0.185\,$\mu$s, respectively; the pipe and socket medians are
12.8--15.0$\times$ higher. In the controlled saturated ablation, the six
...
ring submission overhead (+1.61\,$\mu$s over futex at 64\,B).
...
atomic index updates; the measured adaptive P50 is 211\,ns at 64\,B, more than
...
\begin{enumerate}
  ...
  \item \textbf{Explicit measurement scope.}  We report interrupt-mode
        io\_uring separately from the SQPOLL configurations and label the
        Suite~2 implementation as SQPOLL-assisted signaling because its
        blocking read/wait rings remain interrupt-mode.  Fixed-frequency
        settings are recorded for the low-load wakeup measurements.
...
wakeup signaling path}.  In Suite~1, the consumer first applies a 10\,ms
\texttt{poll()} readiness gate to the named FIFO and then submits an
\texttt{IORING\_OP\_READ}; the tested wait path is therefore
\texttt{poll}/\texttt{io\_uring}, not a pure syscall-free wait.  When the producer detects
...
a stale cleared sleeping flag and skip sending a wakeup,
silently deadlocking the consumer.  Compilers enforce the required global
ordering with architecture-specific locked or fenced sequences; the exact
instruction sequence is compiler- and target-dependent.
...
sleep.  Steps (a) and (b) involve zero system calls; step (c) invokes the
kernel calls required by the selected wait path.
...
structure for Suite~1.  Each primitive is invoked via its standard Linux kernel system
...
64\,B depth-1 P50 is 0.185\,$\mu$s, at the cost of continuously occupying the
...
The consumer first calls \texttt{poll()} with a 10\,ms timeout on a named
FIFO (\texttt{/tmp/\allowbreak uring\-\_sig\-\_fifo}); after readiness, it
submits an \texttt{IORING\-\_OP\-\_READ} SQE and waits through
\texttt{io\-\_uring\-\_submit\-\_and\-\_wait\-(ring,~1)}.  The
...
\begin{table}[!t]
  \centering
\caption{Suite 1 Wakeup Mechanism Properties Summary}
  \label{tab:wakeup}
  \small
  ...
  \begin{tabular}{@{}llll@{}}
    ...
    \texttt{io\_uring}     & poll+submit/wait   & Sleeping        & poll/io\_uring \\
    \bottomrule
  \end{tabular}}
\end{table}
...
Suite~2 uses a separate ping-pong implementation: its io\_uring reader
submits the FIFO read directly through \texttt{io\_uring\_submit\_and\_wait}
and does not use Suite~1's \texttt{poll()} readiness gate.
...
\texttt{data/environment\_pingpong\_primary.txt}.  Core isolation through
\texttt{isolcpus}/\texttt{nohz\_full} was not used; residual scheduler and
idle-state effects therefore remain a limitation.

\textbf{SQPOLL-assisted signal-mode run.}  The completed Suite~2 SQPOLL
configuration was run on the reference native Linux host, with logical CPUs~1 and~2 pinned,
the \texttt{performance} governor, and
\texttt{intel\_pstate.no\_turbo=1}; its record is
\texttt{data/environment\_pingpong.txt}.  It establishes that the SQPOLL
assisted signaling configuration completes under the documented protocol. The
blocking FIFO read/wait rings remain interrupt-mode to avoid a depth-1 SQPOLL
deadlock, so this is not a fully syscall-free end-to-end configuration.
...
\begin{table}[!t]
  \centering
\caption{Single-Trip Ping-Pong Latency Distributions ($\mu$s). Sample counts are 100\,000 (64\,B), 50\,000 (4\,KiB), 10\,000 (64\,KiB), and 2\,000 (1\,MiB). Each row reports P50, P90, P99, P99.9, and mean $\pm$ 95\% CI from the canonical root summary CSVs. SQPOLL denotes the separately labelled SQPOLL-assisted signaling configuration.}
  \label{tab:pingpong}
  \resizebox{\columnwidth}{!}{%
  \small
  \begin{tabular}{@{}lrrrrr@{}}
    ...
    \texttt{shm (busy\_poll)} & \textbf{0.185} & 0.227 & 0.235 & 0.502 & 0.191 $\pm$ 0.001 \\
    \texttt{shm (adaptive)}   & 0.211          & 0.244 & 0.260 & 0.612 & 0.216 $\pm$ 0.001 \\
    \texttt{shm (spin\_back)} & 0.375          & 0.634 & 0.679 & 1.993 & 0.417 $\pm$ 0.001 \\
    \texttt{shm (futex)}      & 2.562          & 2.619 & 3.457 & 4.663 & 2.589 $\pm$ 0.002 \\
    \texttt{shm (eventfd)}    & 2.602          & 3.275 & 4.121 & 5.795 & 2.727 $\pm$ 0.002 \\
    \texttt{pipe}             & 2.710          & 2.773 & 3.787 & 5.827 & 2.747 $\pm$ 0.002 \\
    \texttt{posix\_mq}        & 2.861          & 2.920 & 3.864 & 4.936 & 2.901 $\pm$ 0.012 \\
    \texttt{unix\_socket}     & 2.770          & 4.694 & 5.534 & 7.555 & 3.271 $\pm$ 0.006 \\
    \texttt{shm (io\_uring)}  & 4.173          & 5.275 & 6.156 & 9.433 & 4.422 $\pm$ 0.004 \\
    \texttt{shm (io\_uring SQPOLL)} & 6.912 & 7.051 & 9.417 & 1485.060 & 13.816 $\pm$ 0.570 \\
    \midrule
    \multicolumn{6}{@{}l}{\emph{Message Size: 4\,KiB --- lower is better}} \\[1pt]
    \texttt{shm (adaptive)}   & \textbf{1.124} & 1.160 & 1.219 & 2.867 & 1.130 $\pm$ 0.001 \\
    \texttt{shm (busy\_poll)} & 1.156          & 1.192 & 1.239 & 2.757 & 1.155 $\pm$ 0.001 \\
    \texttt{shm (spin\_back)} & 1.335          & 1.373 & 1.471 & 3.000 & 1.330 $\pm$ 0.001 \\
    \texttt{shm (eventfd)}    & 3.398          & 3.574 & 4.357 & 6.071 & 3.467 $\pm$ 0.030 \\
    \texttt{shm (futex)}      & 3.390          & 4.053 & 5.023 & 8.107 & 3.603 $\pm$ 0.064 \\
    \texttt{pipe}             & 3.634          & 3.824 & 4.937 & 7.293 & 3.719 $\pm$ 0.012 \\
    \texttt{posix\_mq}        & 4.408          & 4.580 & 5.597 & 7.063 & 4.469 $\pm$ 0.021 \\
    \texttt{shm (io\_uring)}  & 5.004          & 5.156 & 6.795 & 8.765 & 5.153 $\pm$ 0.092 \\
    \texttt{unix\_socket}     & 6.063          & 6.428 & 7.118 & 10.059 & 6.058 $\pm$ 0.011 \\
    \texttt{shm (io\_uring SQPOLL)} & 7.590 & 7.902 & 10.787 & 1518.780 & 15.439 $\pm$ 0.853 \\
    \midrule
    \multicolumn{6}{@{}l}{\emph{Message Size: 64\,KiB --- lower is better}} \\[1pt]
    \texttt{shm (busy\_poll)} & \textbf{11.848} & 11.957 & 12.895 & 14.390 & 11.824 $\pm$ 0.008 \\
    \texttt{shm (adaptive)}   & 11.947          & 12.013 & 13.072 & 14.076 & 11.928 $\pm$ 0.008 \\
    \texttt{shm (spin\_back)} & 12.105          & 12.152 & 13.164 & 15.263 & 12.080 $\pm$ 0.008 \\
    \texttt{shm (futex)}      & 13.651          & 14.338 & 16.103 & 20.667 & 13.581 $\pm$ 0.017 \\
    \texttt{shm (eventfd)}    & 14.791          & 21.758 & 33.180 & 110.620 & 16.692 $\pm$ 0.381 \\
    \texttt{unix\_socket}     & 14.793          & 17.163 & 19.825 & 24.540 & 15.713 $\pm$ 0.376 \\
    \texttt{shm (io\_uring)}  & 14.854          & 14.999 & 16.704 & 21.883 & 15.023 $\pm$ 0.300 \\
    \texttt{pipe}             & 18.060          & 18.415 & 20.713 & 24.458 & 18.137 $\pm$ 0.016 \\
    \texttt{posix\_mq}        & N/A             & N/A & N/A & N/A & N/A \\
    \texttt{shm (io\_uring SQPOLL)} & 14.262 & 15.150 & 818.708 & 1623.410 & 28.588 $\pm$ 2.553 \\
    \midrule
    \multicolumn{6}{@{}l}{\emph{Message Size: 1\,MiB --- lower is better}} \\[1pt]
    \texttt{unix\_socket}     & \textbf{122.353} & 125.936 & 129.282 & 134.103 & 123.043 $\pm$ 0.086 \\
    \texttt{shm (busy\_poll)} & 134.712          & 139.938 & 146.149 & 152.020 & 135.735 $\pm$ 0.139 \\
    \texttt{shm (spin\_back)} & 138.311          & 149.811 & 160.231 & 169.930 & 140.585 $\pm$ 0.276 \\
    \texttt{shm (adaptive)}   & 140.755          & 151.248 & 203.329 & 1045.710 & 147.414 $\pm$ 2.376 \\
    \texttt{shm (io\_uring)}  & 150.690          & 193.104 & 244.364 & 373.538 & 162.969 $\pm$ 1.159 \\
    \texttt{shm (eventfd)}    & 163.289          & 182.655 & 201.130 & 232.149 & 164.111 $\pm$ 0.738 \\
    \texttt{shm (futex)}      & 164.144          & 206.372 & 428.928 & 1124.960 & 175.985 $\pm$ 3.055 \\
    \texttt{shm (io\_uring SQPOLL)} & 145.616 & 819.006 & 1773.440 & 1938.180 & 282.603 $\pm$ 16.470 \\
    \texttt{pipe}             & 331.466          & 364.151 & 402.091 & 692.353 & 328.037 $\pm$ 1.937 \\
    \texttt{posix\_mq}        & N/A              & N/A     & N/A     & N/A     & N/A \\
    \bottomrule
  \end{tabular}}
\end{table}
...
\textbf{(1) 12.1--13.8$\times$ spinning advantage at small messages.}
At 64\,B, \texttt{busy\_poll} (0.185\,$\mu$s) and \texttt{adaptive}
(0.211\,$\mu$s) are at least 12$\times$ lower than any kernel-sleeping variant;
busy polling is 13.8$\times$ lower than futex. This
...
\texttt{futex} (2.562\,$\mu$s), \texttt{eventfd} (2.602\,$\mu$s), and
\texttt{io\_uring} (4.173\,$\mu$s) span 2.6--4.2\,$\mu$s P50 at 64\,B.
...
\textbf{(3) POSIX MQ is valid through 4\,KiB in the recorded run.}
The repository's 4\,KiB summary records 50,000 round trips (P50 4.408\,$\mu$s).
The 16\,KiB and larger rows are explicitly marked unsupported by the host
POSIX-MQ message-size limit and are reported as N/A.

\textbf{(4) UNIX Socket wins at 1\,MiB.}
122.35\,$\mu$s vs.\ 134.7--164.1\,$\mu$s P50 for the six primary SHM
...
\textbf{(5) SQPOLL-assisted signaling has low central latency but large tails.}
The SQPOLL-assisted signal-mode run completed all size-scaled Suite~2 samples,
including 10\,000 rounds at 64\,KiB and 2\,000 at 1\,MiB.  Its 64\,B P50 is
6.912\,$\mu$s, but its P99.9 reaches 1.485\,ms.  Since the blocking read/wait
ring remains interrupt-mode to avoid deadlock, this result does not establish
a fully syscall-free SQPOLL end-to-end path.

At 64\,KiB, eventfd's P99/P99.9 (33.180/110.620\,$\mu$s) exceed its
14.791\,$\mu$s P50; the source is untraced and is not attributed to the
primitive.
...
the controlled 64\,KiB results spanning 13.37--15.75\,GiB/s.
...
At 1\,MiB the six controlled variants span 8.12--8.65\,GiB/s.  Payload copying
and the fixed 64-slot ring dominate the wakeup mechanism at this size.

\textbf{(4) Small-message io\_uring overhead at 64\,B.}
At 64\,B payload, the controlled variants span 0.262--0.344\,GiB/s.  This
...
\begin{table}[!t]
  \centering
  \caption{Bursty-Regime Results at 64\,B. Entries are means of the 15
    run-level P50 or CPU-time values.}
  \label{tab:bursty}
  \small
  \resizebox{\columnwidth}{!}{%
...
\begin{table}[!t]
  \centering
  \caption{Offered-Load Sweep at 64\,B: Mean of 15 Run-Level Wakeup
    P50 Values ($\mu$s).}
...
An earlier direct-transport harness recorded a higher 64\,KiB
\texttt{io\_uring} throughput value (28.17\,GiB/s), but it used a different
workload and lacks matching fixed-frequency environment records.  Because the
difference cannot be attributed to a single factor, that result is excluded
from the paper's cross-mechanism conclusions.
...
4.17\,$\mu$s P50, versus \texttt{futex}'s 2.56\,$\mu$s, consistent
...
\begin{itemize}
  \item \textbf{Data path} (dominant): shared memory vs.\ kernel-copy.  It
        determines the observed throughput ordering at message sizes where
        payload transfer dominates.
...
Use SHM ring with \texttt{busy\_poll} or \texttt{adaptive}.  Their 64\,B
P50 values are 185 and 211\,ns, respectively, with P99 below 0.27\,$\mu$s. Busy polling
...
waits and deliver approximately 2.6\,$\mu$s P50 and sub-4.2\,$\mu$s P99 at
...
At 1\,MiB, UNIX Socket achieves 122.4\,$\mu$s P50 versus 134.7--164.1\,$\mu$s
...
At 1\,MiB, the controlled saturated ablation variants span
...
\textbf{No controlled interrupt-mode outlier.}  In the controlled saturated
ablation, interrupt-mode \texttt{io\_uring} reaches 8.12\,GiB/s at 1\,MiB,
within the 8.12--8.65\,GiB/s range of all six variants.
...
\begin{enumerate}
  ...
  \item \textbf{POSIX MQ coverage.}  The recorded configuration includes a
        valid 4\,KiB run.  The 16\,KiB and larger rows are marked
  ...
  \item \textbf{SQPOLL scope.}  SQPOLL is evaluated as a separate 64-slot,
        ...
        multi-FD batching or other SQPOLL thread-affinity choices.  The
        completed Suite~2 SQPOLL-assisted signal-mode run uses the reference
        host, but retains interrupt-mode blocking reads to avoid deadlock. Its
...
\begin{enumerate}
  \item \textbf{The shared-memory data path drives the performance gains.}
        Spinning on a cache-aligned ring delivers 185\,ns P50 at 64\,B.
  ...
  \item \textbf{Spinning variants dominate sub-$\mu$s latency requirements.}
        \texttt{adaptive} balances latency and CPU time: it achieves 211\,ns
        P50 and lowers recorded CPU time during idle periods relative to
        continuous busy polling.
